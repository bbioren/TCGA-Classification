...
import os
# Re-import SentenceTransformerTrainingArguments for gradient accumulation
from sentence_transformers import SentenceTransformerTrainingArguments 

# --- Configuration (Final Stable/Compatible) ---
MODEL_NAME = "jinaai/jina-embeddings-v3"
CSV_FILE = "/root/TCGA-Classification/jina-fine-tune/data.csv"
OUTPUT_DIR = "jina-v3-finetuned-fullcontext-stable"
TASK = "classification"
NUM_EPOCHS = 3
# Reverting to small BATCH_SIZE to avoid OOM when FP16/BF16 is unavailable
BATCH_SIZE = 2
# Compensating for small batch size with accumulation
...
# FIX: num_workers=0 for stability with IterableDataset
...
# Define Training Arguments for gradient accumulation
training_args = SentenceTransformerTrainingArguments(
    output_dir=OUTPUT_DIR,
    num_train_epochs=NUM_EPOCHS,
    per_device_train_batch_size=BATCH_SIZE,
    gradient_accumulation_steps=GRADIENT_ACCUMULATION_STEPS, # Re-introduced for stability
    learning_rate=LEARNING_RATE,
    warmup_ratio=WARMUP_RATIO,
    logging_steps=50,
    # IMPORTANT: All memory flags (fp16, bf16, gradient_checkpointing) are REMOVED 
)
...
# Using model.fit() with compatible arguments
# Note: This relies on the version that accepts 'args='
model.fit(
    train_objectives=[(train_dataloader, train_loss)],
    args=training_args, # This is the accepted way to pass TrainingArguments in this scenario
